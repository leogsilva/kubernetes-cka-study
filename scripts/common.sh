...
command -v bazel version >/dev/null 2>&1 || { \
 echo >&2 "I require Bazel but it's not installed.  Aborting."; exit 1; }

command -v gsutil version >/dev/null 2>&1 || { \
 echo >&2 "I require Bazel but it's not installed.  Aborting."; exit 1; }
...
# get_gce_project() - Returns the current GCE core project config information
# Usage:
#  get_gce_project 
# Returns:
#   the project name

function get_gce_project() {
